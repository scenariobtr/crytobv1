  ...
  return (
    <div className="h-screen bg-black flex flex-col items-center justify-center p-6 relative overflow-hidden">
      <div className="absolute top-10 right-10 z-[70] animate-in fade-in slide-in-from-right-4 duration-1000">
        <button onClick={onToggleLang} className="flex items-center gap-3 px-6 py-3 bg-zinc-900/50 border border-zinc-800 rounded-xl text-[12px] font-black text-neutral-300 hover:text-emerald-500 hover:border-emerald-500/30 backdrop-blur-xl transition-all shadow-2xl group">
      ...
      <div className="text-center space-y-6 relative z-10 w-full max-w-sm">
        ...
        {authView === "LOGIN" && (
          <form onSubmit={onLogin} className="space-y-4 pt-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <button type="button" onClick={() => onAuthViewChange("REGISTER")} className="w-full text-[10px] font-black text-emerald-500/60 uppercase tracking-[0.2em] hover:text-white transition-colors">{t("auth.register_link")}</button>
          </form>
        )}
        ...
        {authView === "REGISTER" && (
          <form onSubmit={onRegister} className="space-y-4 pt-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <button type="button" onClick={() => onAuthViewChange("LOGIN")} className="w-full text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em] hover:text-white transition-colors">กลับหน้าล็อกอิน / BACK TO LOGIN</button>
          </form>
        )}
        ...
        {authView === "OTP" && (
          <form onSubmit={onVerifyOtp} className="space-y-6 pt-4 animate-in fade-in zoom-in duration-500">
            ...
            <button type="button" onClick={() => onAuthViewChange("REGISTER")} className="w-full text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em]">ขอรหัสอีกครั้ง / RESEND</button>
          </form>
        )}
